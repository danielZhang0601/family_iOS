...
void get_sha1_string(char *dest, char *source)
{
	SHA1_CONTEXT ctx;
	sha1_init (&ctx);
	sha1_write (&ctx, (unsigned char *)source, strlen(source));
	sha1_final (&ctx);
	memcpy(dest, (char *)ctx.buf, 20);
	dest[20] = 0;
}

/*
 *dGhlIHNhbXBsZSBub25jZQ==258EAFA5-E914-47DA-95CA-C5AB0DC85B11
 *sha1==>0xb3 0x7a 0x4f 0x2c 0xc0 0x62 0x4f 0x16 0x90
 *  0xf6 0x46 0x06 0xcf 0x38 0x59 0x45 0xb2 0xbe 0xc4 0xea
 *base64_encode ==> s3pPLMBiTxaQ9kYGzzhZRbK+xOo=
 */
void sha1_test()
{
	char dest[64]={0};
	char source[] = "dGhlIHNhbXBsZSBub25jZQ==258EAFA5-E914-47DA-95CA-C5AB0DC85B11";
	get_sha1_string(dest,source);
	char base64[1024] = {0};
	base64_encode(base64, dest, 20);
    for (int i=0; i < 20; i++)  
            printf ("%02x\n", dest[i]);
	puts(base64);
}